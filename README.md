...
*What DeFi did for money, YeBlock LIM does for intelligence.*
...
[![Protocol](https://img.shields.io/badge/protocol-YeBlock%20LIM-9832ff?style=flat-square)](./docs/lim-protocol.md)
...
[Website](https://yeblock.com) · [Architecture](./ARCHITECTURE.md) · [YeBlock LIM Protocol](./docs/lim-protocol.md) · [Concepts](./docs/concepts.md) · [Discussions](https://github.com/yeblocklim/YeBlock/discussions)
...
>
> YeBlock is under active development. The web application is live and accepting waitlist users; the YeBlock LIM protocol, decentralized inference runtime, settlement contracts, and node client are in design and prototyping. This repository is the public documentation hub. Implementation repositories will be opened progressively as components reach public-test maturity.
...
- [What YeBlock LIM Is Not](#what-yeblock-lim-is-not)
...
YeBlock LIM is the alternative.
...
The internet has gone through two decisive unbundlings. YeBlock LIM is the third.
...
| **Web 3 → Web ∞** | **Intelligence** | **YeBlock LIM** | Anyone can serve, compose, monetize cognition. The gatekeepers of AI lose their oligopoly. |
...
**YeBlock LIM (Liquid Intelligence Mesh)** is the protocol stack that powers YeBlock. It is not a product. It is a set of open standards for how inference workloads, model weights, payments, and trust assumptions flow across an open network of independent operators.

A YeBlock LIM-native inference request:
...
YeBlock LIM is to AI inference what **HTTP** was to documents and what **the EVM** was to financial primitives — a thin, opinionated, composable layer that everyone agrees to speak so that nobody has to ask permission.

## The Five Pillars

YeBlock LIM is composed of **five vertically integrated, horizontally composable pillars**. Any application built on YeBlock LIM inherits all five guarantees by default.
...
| 2 | **Decentralized Storage** | Content-addressed model and LoRA weight distribution. Weights are persistent public goods, not vendor inventory. | Eliminates "model deprecation as a business strategy." A weight published to YeBlock LIM remains addressable as long as a single replica exists. |
...
flowchart TB
    ...
    subgraph YeBlockLIM["YeBlock LIM Protocol Stack"]
        direction TB
    ...
    Application --> YeBlockLIM
    YeBlockLIM --> Settlement
    Settlement <--> Network
    YeBlockLIM <--> Network
...
| Dimension | Centralized AI Cloud | **YeBlock LIM** |
|---|---|---|
...
## What YeBlock LIM Is Not

A protocol's identity is sharpened by what it refuses to be. YeBlock LIM is **not**:
...
- **A model marketplace.** YeBlock LIM is a substrate for inference; what models exist on it is decided by authors, not by the protocol.
- **A GPU rental aggregator.** Aggregating idle GPUs is one *consequence* of YeBlock LIM, not its purpose. Without LoRA composability, royalty settlement, and confidential inference, an aggregator is just a billing front-end.
- **An L1 / L2 / app-chain.** YeBlock LIM is chain-agnostic at the protocol layer; it requires a settlement chain but does not specify which one.
...
- **Politically aligned.** YeBlock LIM is policy-neutral. Applications enforce the policies their users want; the protocol does not pre-approve a worldview.
- **A research project.** YeBlock LIM does not invent new cryptographic primitives or training methods. It composes battle-tested standards into a protocol that did not previously exist.

## Why "Liquid Intelligence Mesh"

Three words, deliberately chosen.

- **Liquid** — Capacity, model weights, and economic value flow to where they are most useful, without permission. Idle GPUs become productive inventory; orphaned models become living infrastructure; LoRA authors get paid every time their adapter is composed into an inference. Just as DeFi made money *liquid* across protocols, YeBlock LIM makes intelligence liquid across operators.
- **Intelligence** — The unit of work is reasoning, not blocks or transactions. YeBlock LIM is the first protocol where the primary commodity is *cognition*. Bitcoin priced trust. Ethereum priced computation. YeBlock LIM prices thought.
...
Ahead of those repositories, the **normative reference surface** — the wire data model, composition rules, cryptographic envelopes, and on-chain interfaces every conformant implementation must speak — is published in-repo under [`reference/`](./reference). These are interface definitions and protocol-defined logic transcribed from the spec, not the production engines.
...
| **Phase 1 — Public Documentation** | This repository, architecture spec, YeBlock LIM protocol design | In progress |
...
Apache 2.0 applies to all documentation, diagrams, and code samples in this repository, and is the default license across the YeBlock LIM project — chosen for its explicit patent grant and patent-retaliation clause, which matter for an open protocol intended to be implemented by many independent parties. Implementation repositories (protocol code, SDKs, smart contracts) are released under the same Apache 2.0 unless a domain-specific constraint requires otherwise, in which case the deviation will be stated in that repository.
...
*"DeFi liberated money. YeBlock LIM liberates intelligence."*
